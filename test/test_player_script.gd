...
var _start_point_x: float = 334.0
...
# In this test we press and hold the left button for .1 seconds then wait
# another .3 seconds for the left to take take place.  We then assert that
# the character has moved left arround 25 pixels.
func test_tapping_left_changes_certain_lenght():
	_sender.wait(1.0)
	_sender.action_down("left").hold_for(.1).wait(.3)
	await(_sender.idle)
	assert_between(player.position.x, _start_point_x - 25.0, _start_point_x - 1.0)
	
func test_tapping_right_changes_certain_lenght():
	_sender.wait(1.0)
	_sender.action_down("right").hold_for(.1).wait(.3)
	await(_sender.idle)
	assert_between(player.position.x, _start_point_x + 1, _start_point_x + 25.0)
